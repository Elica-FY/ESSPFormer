...
class ESSPFormerModel(nn.Module):
    def __init__(self, num_classes, encoder_n, input_dim, groupPoolScale=4):
        ...
        self.SPPCFformer_encoders = nn.ModuleList(
            [SPPCFformer(dim=4 * input_dim, depth=1, mlp_dim=32) for _ in range(encoder_n)]
        )
        self.fc0 = nn.Linear(input_dim // groupPoolScale, num_classes)
        self.fc1 = nn.Linear(input_dim//groupPoolScale + 1, num_classes)
    ...
    def forward(self, x):
        ...
        remainder = channels % self.GPScale
        divisible_channels = channels - remainder
        if remainder == 0:
            x = x.view(batch_size, channels // self.GPScale, 4 * self.GPScale).mean(dim=2).cuda()
            x = self.fc0(x)
        else:
