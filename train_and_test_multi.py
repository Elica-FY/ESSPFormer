...
for index_iter in range(args.ITER):
    ...
    file_path = f'./saved_corr_pos/{args.dataset}true_patch{args.patch}_ns{args.n_perGroup}.npy'
    if os.path.exists(file_path):
        print("read")
        total_corr_pos_true = np.load(file_path)[:, -args.n_perGroup - 1:, :]
        total_corr_pos_test = Functions.read_corr(total_corr_pos_true, total_pos_test, args.patch, args.n_perGroup + 1)
        total_corr_pos_train = Functions.corr_data(zero_image, total_pos_train, args.patch, args.pixelNum)
        print(f"total_corr_pos_train  shape: {total_corr_pos_train.shape}")
    else:
        print("calculate")
        total_corr_pos_true = Functions.corr_data(zero_image, total_pos_true, args.patch, args.n_perGroup + 1)
        total_corr_pos_test = Functions.corr_data(zero_image, total_pos_test, args.patch, args.n_perGroup + 1)
        total_corr_pos_train = Functions.corr_data(zero_image, total_pos_train, args.patch, args.pixelNum)
        print(f"total_corr_pos_train  shape: {total_corr_pos_train.shape}")
