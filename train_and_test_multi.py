...
oabi = 4 + (args.patches - args.cnn_patches)//2  # 还要扩大，满足rand的需要
args.batch_size = 256  # 128 64 32 16
args.learning_rate = 5.5e-4
...
for index_iter in range(args.ITER):  # range(10) = [0,1,2...,9]
    ...
    data_hsi, gt_hsi, num_classes, total_pos_train, total_pos_test, total_pos_true, number_train, number_test,\
        number_true = Functions.load_dataset(args.dataset, args.shots, args.VALIDATION_SPLIT,
                                             smallSampling=args.smallTrain)
    ...
    # 1 F149
    mirror_image, zero_image = Functions.mirror_hsi(args.dataset,height, width, band, data_hsi, patch=args.patches,
                                                    cnn_patch=args.cnn_patches)

    # total_corr_pos_true = Functions.corr_data(zero_image, total_pos_true, patches=args.patches,
    #                                           n=args.n_select+1, mod=args.data_mod)
    # total_corr_pos_test = Functions.corr_data(zero_image, total_pos_test, patches=args.patches,
    #                                           n=args.n_select+1, mod=args.data_mod)
    # np.save(f'./save/corr_pos/{args.dataset}true_patch{args.patches}_ns{args.n_select}.npy', total_corr_pos_true)

    total_corr_pos_train = Functions.corr_data(zero_image, total_pos_train, patches=args.patches,
                                               n=args.n_corr, mod=args.data_mod)
    print(f"total_corr_pos_train  shape: {total_corr_pos_train.shape}")
    total_corr_pos_true = np.load(f'./save/corr_pos/{args.dataset}true_patch7_'
                                  f'ns4.npy')[:, -args.n_select-1:, :]

    total_corr_pos_test = Functions.get_saved_corr_data(total_corr_pos_true, total_pos_test, patches=args.patches,
                                                        n=args.n_select+1, mod=args.data_mod)
    r_total_corr_pos_test = Functions.get_saved_corr_data(total_corr_pos_true, total_pos_test, patches=args.patches,
                                                          n=args.n_corr, mod=args.data_mod)

    corr_x_train_band, corr_x_test_band, corr_x_true, nnn_ex = Functions.corr_train_and_test_data(mirror_image, band,
    ...
    # ae = torch.load("D:\\py_code\\MSFormer\\src\\save\\autoencoder\\IN\\IN_64.pt")
    # n_p1,ns,di = corr_x_train_band.shape
    # n_p2 = corr_x_train_band.shape[0]
    # n_p3 = corr_x_train_band.shape[0]
    # from AutoEncoder import AutoEncoder
    # drmlp = AutoEncoder(in_channels=di, out_channels=64)
    # drmlp.load_state_dict(ae, strict=True)
    # encoder = drmlp.encoder
    ...
    x_train = torch.from_numpy(corr_x_train_band.transpose(0, 1, 2)).float()  # [20*n, 200, 3]
    # x_train = encoder(x_train).transpose(2, 0)
    corr_y_train = torch.from_numpy(corr_y_train)  # [20*n]
    Label_train = Data.TensorDataset(x_train, corr_y_train)
    corr_x_test = torch.from_numpy(corr_x_test_band.transpose(0, 1, 2)).float()  # [9671, 200, 3]
    # corr_x_test = encoder(corr_x_test).transpose(2, 0)
    y_test = torch.from_numpy(y_test)  # [9671]
    Label_test = Data.TensorDataset(corr_x_test, y_test)
    x_true = torch.from_numpy(corr_x_true.transpose(0, 1, 2)).float()
    # x_true = encoder(x_true).transpose(2, 0)
    ...
    # -------------------------------------------------------------------------------
    # create model
    model = TransformerModel(input_dim=64, nnss=args.n_select, num_classes=num_classes, n_pool=args.n_pool,
...
if args.smallTrain:
    record_path = f"records/small_sample_{args.smallTrain}/{args.shots}shot_{args.model}__patches{args.patches}_" \
                  f"encoder{args.encoder_num}_cnn_patches{args.cnn_patches}_n_cor{args.n_corr}_n_sel_{args.n_select}" \
                  f"_n_pool{args.n_pool}_{args.dataset}.txt"
else:
    record_path = f"records/small_sample_{args.smallTrain}/atest{args.VALIDATION_SPLIT}test_patches{args.patches}_" \
                  f"cnnpatches{args.cnn_patches}_encoder{args.encoder_num}_ncor{args.n_corr}_ns{args.n_select}" \
                  f"_npool{args.n_pool}_seed{args.seed}{args.dataset}.txt"
