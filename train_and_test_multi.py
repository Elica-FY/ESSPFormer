...
parser.add_argument('--cnn_patches', type=int, default=9, help='cnn滑窗边长')
parser.add_argument('--n_corr', type=int, default=12, help='取相关性前n_corr')
parser.add_argument('--n_select', type=int, default=4, help='取相关性前n_corr中的n_select个')
parser.add_argument('--encoder_num', choices=[1, 2, 3, 4], default=3, help='encoder的数量')
...
args.ITER = 3  # 10
oabi = 4 + (args.patches - args.cnn_patches)//2
...
for index_iter in range(args.ITER):
    ...
    file_path = f'./saved_corr_pos/{args.dataset}true_patch{args.patches}_ns{args.n_select}.npy'
    if os.path.exists(file_path):
        print("read")
        total_corr_pos_true = np.load(file_path)[:, -args.n_select - 1:, :]
        total_corr_pos_test = Functions.read_corr(total_corr_pos_true, total_pos_test, args.patches, args.n_select + 1)
        total_corr_pos_train = Functions.corr_data(zero_image, total_pos_train, args.patches, args.n_corr)
        print(f"total_corr_pos_train  shape: {total_corr_pos_train.shape}")
    else:
        print("calculate")
        total_corr_pos_true = Functions.corr_data(zero_image, total_pos_true, args.patches, args.n_select + 1)
        total_corr_pos_test = Functions.corr_data(zero_image, total_pos_test, args.patches, args.n_select + 1)
        total_corr_pos_train = Functions.corr_data(zero_image, total_pos_train, args.patches, args.n_corr)
        print(f"total_corr_pos_train  shape: {total_corr_pos_train.shape}")
    ...
    x_train = torch.from_numpy(corr_x_train_band.transpose(0, 1, 2)).float()
    corr_y_train = torch.from_numpy(corr_y_train)
    Label_train = Data.TensorDataset(x_train, corr_y_train)
    corr_x_test = torch.from_numpy(corr_x_test_band.transpose(0, 1, 2)).float()
    ...
    # -------------------------------------------------------------------------------
    # create model
    model = ESSPFormerModel(input_dim=band, nnss=args.n_select, num_classes=num_classes, n_pool=args.n_pool,
...
record_path = f"records/a{args.VALIDATION_SPLIT}test_patches{args.patches}_" \
