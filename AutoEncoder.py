...
def generate_batch(X, batch_size=64, shuffle=True):
    ...
    X = X.reshape((row * col, -1)).astype(np.cfloat)
...
def train_ae(X, dataset_name, batch_size=64, epoches=300, lr=1e-2, out_channel=64, decay_step=1000):
...
def IP_experiment(hp: dict):
    dataset_name = 'IN'
    X = load_dataset(dataset_name, 1, 0)[0]
