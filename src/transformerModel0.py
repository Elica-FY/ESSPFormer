...
from src.PreprocessingModule0 import PreprocessingModule
# import numpy as np
import math
...
class MultistrikePooling(nn.Module):
    def __init__(self, input_dim=200):
        ...
        self.num_heads = 3
        self.window_sizes = [4, 8, 16, 24, 32, 48, 64]
        self.stride = 4
    ...
    def forward(self, x):
        ...
        fused_output[:, 0::4] = fused_output0
        fused_output[:, 1::4] = fused_output1
        fused_output[:, 2::4] = fused_output2
        fused_output[:, 3::4] = fused_output4
...
class TransformerModel(nn.Module):
    def __init__(self, num_classes, in_model, cnn_m, en_m, pre_m, encoder_n, cnn_pa, input_dim, nnss, n_pool=1):
        ...
        if cnn_m == 'cov':
            self.conv_or_ave = nn.Conv2d(in_channels=input_dim*5, out_channels=input_dim*5, kernel_size=(cnn_pa, cnn_pa), stride=1, padding=0)
        else:
            self.conv_or_ave = nn.AvgPool2d(kernel_size=(cnn_pa, cnn_pa), stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        batch_size, seq_le, channels, height, width = x.size()
        # print(f'x00  shape:{x.shape}')
        x = x.reshape(-1, height, width)  # 变为 [batch_size* seq_le* channels, 7, 7]
        x = x.reshape(batch_size, seq_le* channels, height, width)
        # print(f'x01  shape:{x.shape}')
        x = self.conv_or_ave(x.float())
        x = x.reshape(batch_size, seq_le, channels).double()
