...
class PreprocessingModule:
    ...
    def ya_preprocess_cube(self, cube, mod='cross-fuse'):
        ...
        if mod == 'cross-fuse':
            ...
            cross_fused_spectra[:, :spectral_dim] = selected_spectra[:, -1, :]
            cross_fused_spectra[:, spectral_dim:spectral_dim*2] = selected_spectra[:, -2, :]
            cross_fused_spectra[:, spectral_dim*2:-spectral_dim] = selected_spectra[:, -3, :]
            cross_fused_spectra[:, -spectral_dim:] = selected_spectra[:, -4, :]  # [:, -4, :]
