...
def load_dataset(Dataset, shots_n, VALIDATION_SPLIT, smallSampling = True, np_random_seed=6):
    if Dataset == 'IN':
    ...
    # normalize data by band norm 光谱维度上归一化，光谱值映射到[0,1]之间
...
#-------------------------------------------------------------------------------
# 边界拓展：镜像
def mirror_hsi(height,width,band,input_normalize,patch,cnn_patch,ds):
    import torch
    ae = torch.load(f"save/autoencoder/{ds}/{ds}_64.pt")
    n_p1, ns, di = input_normalize.shape
    ...
    input_normalize = encoder(torch.from_numpy(input_normalize.transpose(2, 1, 0)).float()).transpose(2, 0)
    # 除法，取向下接近的整数（非四舍五入）
    padding = 4 + patch//2
    mirror_hsi=np.zeros((height+2*padding,width+2*padding,64),dtype=float)  # band = 64
    zero_hsi = np.zeros((height + 2 * padding, width + 2 * padding, 64), dtype=float)  # band = 64
    #中心区域
...
def gain_neighborhood_pixel(mirror_image, point, i, patch=7, oa_bi=0):
    x = point[i,0]
    y = point[i,1]
    temp_image = mirror_image[x+oa_bi:(x+patch+oa_bi), y+oa_bi:(y+patch+oa_bi), :]
    return temp_image
...
def corr_train_and_test_data(mirror_image, corr_pos_train, test_point, true_point, oa_bi, patch=5, nnc=6, nns=3, band = 64):
    n_train, n_corr, _ = corr_pos_train.shape
    x_train = np.zeros((n_train, n_corr, patch, patch, band), dtype=float)
    x_test = np.zeros((test_point.shape[0], nns+1, patch, patch, band), dtype=float)
    x_true = np.zeros((true_point.shape[0], nns+1, patch, patch, band), dtype=float)
...
def combs_exp(x, nc, ns):
    ...
    # 重新排列 x
    X = np.empty((x0 * n_ex, ns+1, x2, x3, x4))
...
def corr_data(mir_gt_hsi, pos_train, patches, n, mod='cor'):
    half_patch = 4 + patches//2
    n_samples = pos_train.shape[0]
    corr_pos_train = np.zeros((n_samples, n, 2), dtype=int)

    if mod == 'repeat':
        corr_pos_train = np.repeat(pos_train[:, np.newaxis, :], n, axis=1)
    else:
        # 遍历每一个样本点
        for i in range(n_samples):
            ...
            if mod == 'cor':
                ...
                top_coords = np.concatenate([np.random.randint(0, patches, size=(n - 1, 2)), np.zeros((1, 2)) + patches//2], axis=0)
            # 转换为全局坐标
