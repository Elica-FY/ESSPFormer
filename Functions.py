...
def load_dataset(Dataset, shots_n, VALIDATION_SPLIT, smallSampling = False, np_random_seed=6):
    if Dataset == 'IN':
    ...
    if mod == 'repeat':
        ...
        for i in range(n_samples):
            ...
            if mod == 'cor':
                ...
                top_coords = np.concatenate(
                    [np.ones((1, 2)) + patches // 2, np.random.randint(0, patches, size=(n - 1, 2))], axis=0)
            # 转换为全局坐标
...
def get_saved_corr_data(pos_true, pos_train, patches, n, mod='cor'):
    half_patch = 4 + patches//2
    n_samples = pos_train.shape[0]
    corr_pos_train = np.zeros((n_samples, n, 2), dtype=int)
    if mod == 'cor':
        ...
        # # 验证结果
        # print("Shape of total_corr_pos_test:", corr_pos_train.shape)
    elif mod == 'repeat':
        corr_pos_train = np.repeat(pos_train[:, np.newaxis, :], n, axis=1)
    else:
        # 遍历每一个样本点
