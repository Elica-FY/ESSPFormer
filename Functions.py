...
def corr_train_and_test_data(mirror_image, band, corr_pos_train, test_point, true_point, oa_bi, hpfpatch, pN, npG):
    n_train, n_corr, _ = corr_pos_train.shape
    x_train = np.zeros((n_train, n_corr, band), dtype=float)
    x_test = np.zeros((test_point.shape[0], npG+1, band), dtype=float)
    x_true = np.zeros((true_point.shape[0], npG+1, band), dtype=float)
    for i in range(n_train):
        for j in range(n_corr):
            vc = gain_neighborhood_pixel(mirror_image, corr_pos_train[i], j, hpfpatch, oa_bi)
            x_train[i, j, :] = vc
    x_train_band, nn_ex = combs_exp(x_train, pN, npG)
    for i in range(test_point.shape[0]):
        for j in range(npG+1):
            x_test[i, j, :] = gain_neighborhood_pixel(mirror_image, test_point[i], j, hpfpatch, oa_bi)
    x_test[:, :-1, :] = x_test[:, 1:, :]
    for i in range(true_point.shape[0]):
        for j in range(npG+1):
            x_true[i, j, :] = gain_neighborhood_pixel(mirror_image, true_point[i], j, hpfpatch, oa_bi)
    x_true[:, :-1, :] = x_true[:, 1:, :]
